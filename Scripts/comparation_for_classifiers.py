...
class Run:
    ...
    def run_all_classifiers(self, file_info, pgaOn='no', baseOrFlip='base', stage='0'):
        ...
        for inx in range(1):
            ...
            name_linear_random = nd.linear_svm(train_data, test_data, tr_label_name, te_label_name, random_label=True)
            cate_linear_random = nd.linear_svm(train_data, test_data, tr_label_category, te_label_category, random_label=True)
            # print('----------------------------------------------Accuracies for different object----------------------------------------------------')
            name_linear_accPerObj, _ = nd.accuracy_per_object(train_data, test_data, tr_label_name, te_label_name)
            cate_linear_accPerObj, _ = nd.accuracy_per_object(train_data, test_data, tr_label_category, te_label_category)
    ...
    def main(self, pgaOn='no', baseOrFlip='base'):
        ...
        # directions = ['D:\\Data\OCT_TDT\pre_stimuli\Stage1\data_PGA']
