        ...
        for dtype in self.settings['epochTypes']:
            ...
            for ch in range(self.numberOfChannels):
                if ch not in self.settings['badChannels']:
                    ...
                    #
                    #         corrMatRec = np.corrcoef(meanSigs) - np.eye(meanSigs.shape[0])
                    #         loc = np.unravel_index(corrMatRec.argmin(), corrMatRec.shape)
                    #
                    #         if corrMatRec[loc[0], loc[1]] < 0:
                    #             dataFlip[idx == trans[loc[1]+1]] = -dataFlip[idx == trans[loc[1]+1]]
                    #             meanSigs[loc[1]+1,:] = -meanSigs[loc[1]+1,:]
                    #
                    #         corrMatRecNew = np.corrcoef(meanSigs) - np.eye(meanSigs.shape[0])
                    #         if np.sum(corrMatRecNew) < np.sum(corrMatRec):
                    #             dataFlip[idx == trans[loc[1]+1]] = -dataFlip[idx == trans[loc[1]+1]]
                    #             flag = 0
